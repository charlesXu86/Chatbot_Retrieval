...
@File    :   run_similarity_albert.py

@Time    :   2019-12-19 14:27

@Desc    :   基于albert的语义相似度匹配
...
from Chatbot_Retrieval_model.src_code import modeling
from Chatbot_Retrieval_model.src_code import tokenization
from Chatbot_Retrieval_model.src_code import optimization

import tensorflow as tf
from tensorflow.python.estimator.model_fn import EstimatorSpec

from Chatbot_Retrieval_model.Bert_sim.config_albert import Config
...
class AlBertSim():
    ...
    def get_estimator(self):
        ...
        albert_config = modeling.AlbertConfig.from_json_file(cf.albert_config_file)
    ...
    def create_model(self, albert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    def model_fn_builder(self, albert_config, num_labels, init_checkpoint, learning_rate,
        ...
        def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
            ...
            (total_loss, per_example_loss, probabilities, logits, predictions) = self.create_model(
                                                                 albert_config, is_training, input_ids, input_mask,
    ...
    def convert_examples_to_features2(self, examples, label_list, max_seq_length,
                                     tokenizer):
        """Convert a set of `InputExample`s to a list of `InputFeatures`."""
    ...
    def train(self):
        ...
        if cf.max_seq_length > albert_config.max_position_embeddings:
            raise ValueError(
                "Cannot use sequence length %d because the ALBERT model "
                "was only trained up to sequence length %d" %
                (cf.max_seq_length, albert_config.max_position_embeddings))
        tf.gfile.MakeDirs(cf.output_dir)

        train_examples = self.processor.get_train_examples(cf.data_dir)
        train_file = os.path.join(cf.output_dir, cf.task_name + "_train.tf_record")
        label_list = self.processor.get_labels()

        iterations_per_loop = int(min(cf.iterations_per_loop, cf.save_checkpoints_steps))

        estimator = self.get_estimator()

        self.file_based_convert_examples_to_features(
                train_examples, label_list, cf.max_seq_length, self.tokenizer, train_file)

        tf.compat.v1.logging.info("***** Running training *****")
        tf.compat.v1.logging.info("  Num examples = %d", len(train_examples))
        tf.compat.v1.logging.info("  Batch size = %d", cf.batch_size)
        tf.compat.v1.logging.info("  Num steps = %d", iterations_per_loop)
        train_input_fn = self.file_based_input_fn_builder(input_file=train_file, seq_length=cf.max_seq_length,
                                                          is_training=True,
                                                          drop_remainder=True,
                                                          bsz=cf.batch_size
                                                          )
        estimator.train(input_fn=train_input_fn, max_steps=cf.train_step)
...
#     albert_sim = AlBertSim()
#     if cf.do_train:
#         albert_sim.set_mode(tf.estimator.ModeKeys.TRAIN)
#         albert_sim.train()
#         albert_sim.set_mode(tf.estimator.ModeKeys.EVAL)
#         albert_sim.eval()
#     if cf.do_predict:
#         albert_sim.set_mode(tf.estimator.ModeKeys.PREDICT)
#         sentence1 = '你多大了'
#         sentence2 = '成都哪里好玩'
#         predict = albert_sim.predict(sentence1, sentence2)['probabilities'].tolist()[0]
#         print(predict)
