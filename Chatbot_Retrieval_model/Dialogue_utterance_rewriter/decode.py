...
from Chatbot_Retrieval_model.Dialogue_utterance_rewriter import beam_search
...
def get_decode_dir_name(ckpt_name):
    ...
    if "train" in cf.data_dir: dataset = "train"
    elif "val" in cf.data_dir: dataset = "val"
    elif "test" in cf.data_dir: dataset = "test"
    else:
        raise ValueError(
            "cf.data_path %s should contain one of train, val or test" %
            (cf.data_dir))
    dirname = "decode_%s_%imaxenc_%ibeam_%imindec_%imaxdec" % (
        dataset, cf.max_enc_steps, cf.beam_size, cf.min_dec_steps,
        cf.max_dec_steps)
