...
@Desc    :   1、This file contains code to process data into batches
...
from Chatbot_Retrieval_model.Dialogue_utterance_rewriter.data import *
...
class Example(object):
    ...
    def __init__(self, context, summarization, query, vocab, hps):
        ...
        summarization_words = summarization
...
class Batcher(object):
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                (context, summarization, query) = next(input_gen)
        ...
        for f in filelist:
            with open(f) as train_f:
                for line in train_f:
                    ...
                    if len(record) != 4:
                        continue
                    yield (record[0].strip()+ MARK_EOS +  record[1].strip(), record[2].strip(), record[3].strip())
        if single_pass:
