...
from Bert_sim.config import Config
...
def model_fn_builder(bert_config, num_labels, train_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if train_checkpoint:
            (assignment_map, initialized_variable_names
             ) = modeling.get_assignment_map_from_checkpoint(tvars, train_checkpoint)
            if use_tpu:

                def tpu_scaffold():
                    tf.train.init_from_checkpoint(train_checkpoint, assignment_map)
...
def predicts(text_data):
    ...
    tokenization.validate_case_matches_checkpoint(cf.do_lower_case,
                                                  cf.train_checkpoint)
    ...
    model_fn = model_fn_builder(
        ...
        train_checkpoint=cf.train_checkpoint,
    ...
    if cf.do_predict:
        # predict_examples = processor.get_test_examples(cf.data_dir)
        test_data = []
        for index in range(len(text_data)):
            guid = 'test-%d' % index
            text_a = tokenization.convert_to_unicode(str(text_data[index][0]))
            text_b = tokenization.convert_to_unicode(str(text_data[index][1]))
            # label = str(test[2])
            test_data.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=None))
        ...
        predict_drop_remainder = True if cf.use_tpu else False
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                res.append([dicts[0][0], dicts[0][1]])
