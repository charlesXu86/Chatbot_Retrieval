...
def main():
  ...
  if cf.do_train:
    train_examples = processor.get_train_examples(cf.train_file)
    num_train_steps = int(
  ...
  if cf.do_eval:
    eval_examples = processor.get_dev_examples(cf.predict_file)
    ...
    if cf.use_tpu:
      ...
      # the last batch.
      eval_steps = int(len(eval_examples) / cf.eval_batch_size)

    eval_drop_remainder = True if cf.use_tpu else False
  ...
  if cf.do_predict:
    predict_examples = processor.get_test_examples(cf.test_file)
